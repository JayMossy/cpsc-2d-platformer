...
import { applyGravity, clampFallSpeed, setMovementX, integrate } from "./physics.js";
...
export function playerMovement(dt) {
    ...
    if (player.vx !== 0) {
        if (player.lastDir === "left")
            animator.setAnimation("run left");
        else
            animator.setAnimation("run right");
    }
    else {
        if (player.lastDir === "left")
