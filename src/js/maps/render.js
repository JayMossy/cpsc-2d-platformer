...
// import { playerAnimator } from "./playerMovement.js";
// import {
...
const tempMapRender = new TempMap();
...
export function render() {
    ...
    // I'm thinking when we have all our maps we can just export this one "super" render function
    // and some function inside here that handles when to switch maps
    tempMapRender.render();
    ...
    // tempMapRender.updateCamera();
    // tempMapRender.drawMap();

    // // Easier debug we can see out "hit box"
    // ctx.strokeStyle = "red";
    // ctx.strokeRect(
    //     player.x - tempMapRender.camera.x,
    //     player.y - tempMapRender.camera.y,
    //     player.w,
    //     player.h
    // )
    // const playerCol = Math.floor(player.x / tileSize);
    // const playerRow = Mrows - Math.floor((player.y + player.h + 0.1) / tileSize);
    ...
    //     ctx.fillStyle = "red";
    //     ctx.fillRect(enemy.x - tempMapRender.camera.x, enemy.y - tempMapRender.camera.y, enemy.w, enemy.h);
    // }

    // coins.forEach(coin => {
    //     coin.draw(ctx, tempMapRender.camera);
